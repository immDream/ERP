    ...
    >
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <el-button :type="step === 3 ? 'success' : 'primary'"
                       @click="onNextBtnClick" >{{step === 3 ? '确认修改' : '下一步'}}
            </el-button>
        </span>
    </el-dialog>
</template>
...
<script>
    ...
    import {UPDATE_PRODUCT_INFO} from "@/store/mutation-types";
    export default {
        name: "UpdateDialog",
        components: {MyTag},
        props: ["visible", "data"],
        ...
        watch: {
            "visible": function (visible) {
                if(visible) {
                    let pData = JSON.parse(JSON.stringify(this.$props.data));
                    this.categoriesOptions = pData.categoriesOptions;
                    // 获取对应的品牌列表
                    this.onCategoriesChange(pData.categories);
                    // 设定基本信息
                    this.ruleForm0 = {
                    ...
                    pData.sku.skuList.forEach(value => {
                        if(value.id > currSkuId) currSkuId = value.id;
                    });
                    this.currSkuId = currSkuId;
                    // 设定SKU属性
                    this.skuList = pData.sku.skuList;
                    this.ruleForm2 = {
                        ...pData.sku.data,
                        isInit: true
                    };
                    // 设定价格
                    this.skuGroupList = pData.prices;
                    this.resetGroupList = JSON.parse(JSON.stringify(pData.prices));
                }
            },
            /**
             * 监听sku属性变化
        ...
        methods: {
            ...
            onNextBtnClick: function () {
                ...
                if(this.step === 0) {
                    ...
                    if(!validate) {
                        ...
                        // });
                        // 执行修改逻辑
                        this.$store.dispatch(UPDATE_PRODUCT_INFO, {
                            ...this.ruleForm0,
                            // brandName,
                            spu: this.ruleForm1,
                            sku: {
            ...
            onCategoriesChange: function (value) {
                ...
                this.$axios.post('/allBrandNameList', {
                    ...
                    .then(response => {
                        ...
                        this.getSpecificationList(value, true);
                    })
                    .catch(error => {
             ...
             * @param value
             */
            getSpecificationList: function (value, isInit) {
                // 清空原本的ruleForm1表单数据
                if(!isInit) this.ruleForm1 = {};
                this.$axios.post("/specificationList", {
                    ...
                    .then(response => {
                        ...
                        list.forEach((value, index) => {
                            ...
                            // 动态生成表单属性 curVal 初始值
                            if(isInit) {
                                this.ruleForm1 = JSON.parse(JSON.stringify(this.$props.data)).spu;
                            } else {
            ...
            createSkuGroupList: function () {
                ...
                for(let s in this.ruleForm2) {
                    if(Array.isArray(this.ruleForm2[s]))
